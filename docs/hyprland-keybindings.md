...
Hyprland is available from SDDM on `bravo` and `quebec`. The shared configuration uses the Super key as its primary modifier.
...
Groups are tabbed containers; their tabs appear above the grouped window.

| Key | Action |
| --- | --- |
| `Super + G` | Create or dissolve a group for the focused window |
| `Super + Tab` | Focus the next window in the group |
| `Super + Shift + Tab` | Focus the previous window in the group |
| `Super + Alt + H/J/K/L` | Move the focused window into the group left/down/up/right, creating one if needed |
| `Super + Ctrl + G` | Remove the focused window from its group |
...
| Key | Action |
| --- | --- |
| `Print` | Select, save, and copy a region |
